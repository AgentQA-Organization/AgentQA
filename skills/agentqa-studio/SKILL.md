...
- `uploads/` — requirements documents attached to a job (read-only to you)
...
step 3. **If the record carries `requirements`, read that file first** — see
below. On `{"status":"waiting"}` the 8-minute poll simply timed out — **run the
...
### 2b. An attached requirements document

A job can carry a document the user uploaded with it:

```json
"requirements": {"name": "SRD-login.md", "path": ".agentqa/studio/uploads/ab12cd34-SRD-login.md", "chars": 4210}
```

Read it at **step 0**, in the same pass as `config.yml`'s `docs:` artifacts, and
treat it as exactly that: **one more `docs:` artifact, scoped to this job.**
Everything `agentqa-write-test` already says about product artifacts applies to
it unchanged —
[`references/memory-model.md`](../agentqa-write-test/references/memory-model.md)'s
Intent layer and
[`references/clarify.md`](../agentqa-write-test/references/clarify.md):
...
- **It pre-fills the clarify card, it never replaces it.** Where the document
  ...
  field's `default` so the user confirms-or-corrects instead of retyping. All
  four questions are still asked. A document never removes a question.
...
- The file is inside the gitignored mailbox and is **read-only** to you.

Post a `progress` line naming the file so the browser shows it was actually
used:
...
If the path does not resolve, don't abort — say so in a `progress` line and run
the job as if no document were attached.
...
differences are steps 2b above and 4 below.
